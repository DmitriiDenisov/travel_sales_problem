...
if os.path.exists(results_file):
    ...
    results_dict = {
        ...
        'strict_ant_colony': {'times': [], 'results': []},
        'n_values': [],
        'deviations_ant_colony': [],
        'deviations_convex_hull': [],
        'deviations_greedy': [],
        'deviations_strict_ant_colony': []
    }

n_values = list(range(3, 14))
...
for n in n_values[start_index:]:
    ...
    # Выполнение и сохранение результатов муравьиного алгоритма с более строгими параметрами
    strict_params = lambda pts: ant_colony_solver(pts, iterations=200, n_ants=20, alpha=1, beta=2, rho=0.5, Q=100)
    time_strict_ant, result_strict_ant = measure_time_and_result(strict_params, points)
    results_dict['strict_ant_colony']['times'].append(time_strict_ant)
    results_dict['strict_ant_colony']['results'].append(result_strict_ant)
    ...
    deviation_strict_ant_colony = abs(result_strict_ant[0] - result_bnb[0])
    ...
    results_dict['deviations_strict_ant_colony'].append(deviation_strict_ant_colony)
...
fig.add_trace(go.Scatter(x=results_dict['n_values'], y=results_dict['deviations_strict_ant_colony'], mode='lines+markers', name='Strict Ant Colony Deviation', line=dict(width=3)))
